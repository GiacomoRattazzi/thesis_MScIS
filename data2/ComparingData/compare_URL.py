...
for json_file_name in tqdm(os.listdir(json_dir)):
    ...
    with open(json_file_path, 'r') as file:
        data = json.load(file)
        json_urls[data['url']] = json_file_name
...
for csv_file_name in tqdm(os.listdir(csv_dir), position = 0):
    print("Comparing URLs for file " + str(i) + "...")
    i+=1
    csv_file_path = os.path.join(csv_dir, csv_file_name)
    csv_data = pd.read_csv(csv_file_path)
    for index, row in csv_data.iterrows():
        if row['url'] in json_urls:
            matches.append({
                'csv_id': row['id'],
                'json_file': json_urls[row['url']],
                'csv_file': csv_file_name,
                'csv_index': index
            })


pd.DataFrame(matches).to_csv('matches_URL.csv', index=False)
